...
async function loadOverview() {
  try {
    const res = await fetch(`${API_BASE}/reconciliation`, { headers: authHeaders() });
    ...
    document.getElementById('statCashReconciliation').textContent = formatCurrency(s.cashCollected);
    document.getElementById('statOnlineReconciliation').textContent = formatCurrency(s.onlineCollected);
    document.getElementById('statOutstandingDues').textContent = formatCurrency(s.totalDue);
    ...
    if (s.doctorBreakdown && s.doctorBreakdown.length > 0) {
      docBody.innerHTML = s.doctorBreakdown.map(doc => `
        <tr>
      ...
      docBody.innerHTML = '<tr><td colspan="3" class="empty-cell">No appointment charges processed.</td></tr>';
    }
    ...
    if (s.recentActivity && s.recentActivity.length > 0) {
      invoicesBody.innerHTML = s.recentActivity.map(inv => `
        <tr>
      ...
      invoicesBody.innerHTML = '<tr><td colspan="4" class="empty-cell">No recent invoices logged.</td></tr>';
    }
  } catch (err) {
...
async function loadPatients() {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data.success) {
      tbody.innerHTML = '<tr><td colspan="7" class="empty-cell">Error loading patients</td></tr>';
      return;
    }

    const patients = data.patients || [];
    if (patients.length === 0) {
      tbody.innerHTML = '<tr><td colspan="7" class="empty-cell">No patients found. Click "+ Add Patient" to register one.</td></tr>';
      renderPagination('patientsPagination', 1, 1, 1, (p) => { patientPage = p; loadPatients(); });
      return;
    }
    ...
    const pag = data.pagination || { page: 1, totalPages: 1, total: patients.length };
    renderPagination('patientsPagination', pag.page, pag.totalPages, pag.total, (p) => {
      patientPage = p;
      loadPatients();
    });
  } catch (err) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty-cell">Failed to retrieve patient logs.</td></tr>';
  }
}
...
async function savePatient(e) {
  ...
  try {
    ...
    if (res.ok && result.success) {
      showToast(id ? 'Patient profile updated successfully!' : 'New patient registered!', 'success');
...
window.deletePatient = async function(id) {
  if (!confirm('Are you sure you want to permanently delete this patient? This will delete all linked visits and bills!')) return;
  try {
...
async function loadAppointments() {
  ...
  tbody.innerHTML = '<tr><td colspan="9" class="loading-cell"><span class="spinner"></span> Querying visit slots...</td></tr>';

  try {
    let url = `${API_BASE}/appointments?page=${appointmentPage}&limit=10`;
    if (appointmentSearch) url += `&search=${encodeURIComponent(appointmentSearch)}`;
    if (appointmentDate) url += `&date=${encodeURIComponent(appointmentDate)}`;

    const res = await fetch(url, { headers: authHeaders() });
    const data = await res.json();
    if (!res.ok || !data.success) {
      tbody.innerHTML = '<tr><td colspan="9" class="empty-cell">Error loading appointments</td></tr>';
      return;
    }

    const appointments = data.appointments || [];
    if (appointments.length === 0) {
      tbody.innerHTML = '<tr><td colspan="9" class="empty-cell">No appointments scheduled for selected parameters.</td></tr>';
      renderPagination('appointmentsPagination', 1, 1, 0, (p) => { appointmentPage = p; loadAppointments(); });
      return;
    }
    ...
    const pag = data.pagination || { page: 1, totalPages: 1, total: appointments.length };
    renderPagination('appointmentsPagination', pag.page, pag.totalPages, pag.total, (p) => {
      appointmentPage = p;
      loadAppointments();
    });
  } catch (err) {
    tbody.innerHTML = '<tr><td colspan="9" class="empty-cell">Failed to retrieve scheduling.</td></tr>';
  }
}
...
async function loadPatientsDropdown(selectId) {
  ...
  try {
    const res = await fetch(`${API_BASE}/patients?limit=100`, { headers: authHeaders() });
    const data = await res.json();
    if (res.ok && data.success) {
...
async function saveAppointment(e) {
  ...
  try {
    ...
    const result = await res.json();
    if (res.ok && result.success) {
      showToast(id ? 'Appointment modified!' : 'Visit scheduled and bill logged!', 'success');
...
window.editAppointment = async function(id) {
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      document.getElementById('appointmentModalTitle').textContent = 'Modify Scheduled Slot';
      ...
      document.getElementById('app_date').value = a.appointment_date.split('T')[0];
      ...
      document.getElementById('autoInvoiceCheckboxGroup').style.display = 'none';
...
window.completeAppointment = async function(id) {
  if (!confirm('Mark this appointment as Completed? If linked to an unpaid invoice, you should reconcile billing.')) return;
  try {
    const res = await fetch(`${API_BASE}/appointments/${id}`, { headers: authHeaders() });
    const data = await res.json();
    if (res.ok && data.success) {
...
window.deleteAppointment = async function(id) {
  if (!confirm('Permanently delete this scheduled slot? Linked invoices remain but appointment link is cleared.')) return;
  try {
...
async function loadInvoices() {
  ...
  tbody.innerHTML = '<tr><td colspan="10" class="loading-cell"><span class="spinner"></span> Processing bills ledger...</td></tr>';
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok || !data.success) {
      tbody.innerHTML = '<tr><td colspan="10" class="empty-cell">Error loading billing ledger</td></tr>';
      return;
    }

    const invoices = data.invoices || [];
    if (invoices.length === 0) {
      tbody.innerHTML = '<tr><td colspan="10" class="empty-cell">No billing invoice logs recorded.</td></tr>';
      renderPagination('invoicesPagination', 1, 1, 0, (p) => { invoicePage = p; loadInvoices(); });
      return;
    }
    ...
    tbody.innerHTML = invoices.map(i => {
      ...
      if (i.status === 'paid') statusBadge = `<span class="badge badge-paid">paid</span>`;
      else if (i.status === 'partially_paid') statusBadge = `<span class="badge badge-partial">partial</span>`;
    ...
    const pag = data.pagination || { page: 1, totalPages: 1, total: invoices.length };
    renderPagination('invoicesPagination', pag.page, pag.totalPages, pag.total, (p) => {
      invoicePage = p;
      loadInvoices();
    });
  } catch (err) {
    tbody.innerHTML = '<tr><td colspan="10" class="empty-cell">Failed to fetch invoices ledger.</td></tr>';
  }
}
...
window.openReconciliationModal = function(id, invNo, amount, due) {
  ...
  document.getElementById('reconTotalBilled').textContent = formatCurrency(amount);
...
async function processReconciliation(e) {
  ...
  const totalAmount = parseFloat(document.getElementById('recon_invoice_amount').value);
  const payAmount = parseFloat(document.getElementById('recon_paid_amount').value);
  ...
  try {
    ...
    const fetchRes = await fetch(`${API_BASE}/invoices/${id}`, { headers: authHeaders() });
    ...
    if (updateRes.ok && result.success) {
      showToast(`Reconciled ₹ ${payAmount} paid in ${mode.toUpperCase()}!`, 'success');
...
async function saveStaff(e) {
  ...
  try {
    ...
    const result = await res.json();
    if (res.ok && result.success) {
      showToast(id ? 'Staff credentials updated!' : 'Staff account created successfully!', 'success');
...
window.editStaff = async function(id) {
  try {
    ...
    if (res.ok && data.success) {
      ...
      document.getElementById('staff_password').placeholder = 'Enter password only to reset';
      ...
      document.getElementById('saveStaffBtn').textContent = 'Update Credentials';
...
window.deleteStaff = async function(id) {
  if (!confirm('Are you sure you want to delete this staff login? They will lose all access.')) return;
  try {
...
function initEventListeners() {
  ...
  document.getElementById('bookAppointmentBtn').addEventListener('click', async () => {
    ...
    document.getElementById('appointmentModalTitle').textContent = 'Schedule Visit';
    ...
    document.getElementById('autoInvoiceCheckboxGroup').style.display = 'flex';
  ...
  document.getElementById('createInvoiceBtn').addEventListener('click', async () => {
    ...
    document.getElementById('invoiceModalTitle').textContent = 'Create Custom Invoice';
  ...
  document.getElementById('addStaffBtn').addEventListener('click', () => {
    ...
    document.getElementById('staffModalTitle').textContent = 'Create Staff Account';
    
    document.getElementById('staff_password').required = true;
    document.getElementById('staff_password').placeholder = 'Enter secure password';
    document.getElementById('staffPassHint').style.display = 'none';
    document.getElementById('staffPassLabel').innerHTML = 'Password <span class="req">*</span>';
  ...
  document.getElementById('inv_status').addEventListener('change', (e) => {
    ...
    document.getElementById('invPaymentModeGroup').style.display = showPayMode ? 'block' : 'none';
  });
  ...
  document.getElementById('filterAppointmentDate').addEventListener('change', (e) => {
  ...
  document.getElementById('searchInvoiceInput').addEventListener('input', (e) => {
  ...
  document.getElementById('filterInvoiceStatus').addEventListener('change', (e) => {
...
function renderPagination(containerId, currentPage, totalPages, totalItems, onPageChange) {
  ...
  if (totalPages <= 1) {
    container.innerHTML = totalItems > 0 ? `<span style="font-size:12px; color:var(--text3);">Showing ${totalItems} records</span>` : '';
    return;
  }
