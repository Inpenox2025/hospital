...
module.exports = async function handler(req, res) {
  ...
  if (action === 'export-pdf') {
    ...
    if (!pdfId) return res.status(400).json({ error: 'Invoice ID is required (?id=X)' });
    ...
    try {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="Receipt_${invoice.invoice_no}.pdf"`);
      doc.pipe(res);
      ...
      doc.fontSize(9).font('Helvetica').fillColor('#64748b')
         .text('24/7 Patient Care, Consultation & Diagnostics', { align: 'center' });
      doc.fontSize(9).text('Contact: +91 9988776655 | Email: support@radhagovindcare.com', { align: 'center' });
      doc.moveDown(0.8);

      // Decorative divider
      doc.moveTo(50, doc.y).lineTo(545, doc.y).strokeColor('#0d9488').lineWidth(2).stroke();
      doc.moveDown(1.2);
      ...
      doc.moveTo(50, doc.y + 2).lineTo(250, doc.y + 2).strokeColor('#cbd5e1').lineWidth(1).stroke();
      doc.moveTo(320, doc.y + 2).lineTo(545, doc.y + 2).strokeColor('#cbd5e1').lineWidth(1).stroke();
      doc.moveDown(0.6);

      // Patient details left column
      const detailsY = doc.y;
      doc.fontSize(10).font('Helvetica-Bold').fillColor('#475569');
      doc.text('Name: ', 50, detailsY, { continued: true }).font('Helvetica').fillColor('#0f172a').text(invoice.patient_name);
      doc.font('Helvetica-Bold').fillColor('#475569').text('Mobile: ', { continued: true }).font('Helvetica').fillColor('#0f172a').text(invoice.patient_mobile || '—');
      doc.font('Helvetica-Bold').fillColor('#475569').text('Address: ', { continued: true }).font('Helvetica').fillColor('#0f172a').text(invoice.patient_address || '—');

      // Invoice details right column
      doc.text('Receipt No: ', 320, detailsY, { continued: true }).font('Helvetica').fillColor('#0f172a').text(invoice.invoice_no);
      doc.font('Helvetica-Bold').fillColor('#475569').text('Date Issued: ', { continued: true }).font('Helvetica').fillColor('#0f172a').text(formatDate(invoice.created_at));
      
      const pMode = invoice.payment_mode ? invoice.payment_mode.toUpperCase() : 'PENDING';
      doc.font('Helvetica-Bold').fillColor('#475569').text('Payment Mode: ', { continued: true }).font('Helvetica').fillColor('#0f172a').text(pMode);
      
      const pDate = invoice.payment_date ? formatDate(invoice.payment_date) : '—';
      doc.font('Helvetica-Bold').fillColor('#475569').text('Payment Date: ', { continued: true }).font('Helvetica').fillColor('#0f172a').text(pDate);

      doc.moveDown(1.5);

      // Medical Details (if appointment exists)
      if (invoice.doctor_name) {
        doc.fontSize(10).font('Helvetica-Bold').fillColor('#334155').text('CLINICAL VISIT DETAILS', 50);
        doc.moveTo(50, doc.y + 2).lineTo(545, doc.y + 2).strokeColor('#cbd5e1').lineWidth(1).stroke();
        doc.moveDown(0.6);

        const clinicalY = doc.y;
        doc.font('Helvetica-Bold').fillColor('#475569').text('Consultant: ', 50, clinicalY, { continued: true }).font('Helvetica').fillColor('#0f172a').text(invoice.doctor_name);
        doc.font('Helvetica-Bold').fillColor('#475569').text('Visit Date: ', { continued: true }).font('Helvetica').fillColor('#0f172a').text(formatDate(invoice.appointment_date) + ' ' + (invoice.appointment_time || ''));
        doc.moveDown(1.5);
      }

      // Invoice Items Table
      doc.fontSize(10).font('Helvetica-Bold').fillColor('#334155').text('BILLING DESCRIPTION & CHARGES', 50);
      doc.moveTo(50, doc.y + 2).lineTo(545, doc.y + 2).strokeColor('#cbd5e1').lineWidth(1).stroke();
      doc.moveDown(0.6);
      ...
      doc.text(invoice.description || 'Medical Consultation Fee', 50, tableRowY, { width: 380 });
      doc.text(formatCurrency(invoice.amount), 450, tableRowY, { align: 'right', width: 95 });
      doc.moveDown(1.5);

      // Totals & Balances
      doc.moveTo(300, doc.y).lineTo(545, doc.y).strokeColor('#cbd5e1').lineWidth(1).stroke();
      doc.moveDown(0.6);

      const totalsY = doc.y;
      doc.font('Helvetica-Bold').fillColor('#475569').text('Total Amount:', 320, totalsY);
      doc.font('Helvetica-Bold').fillColor('#0f172a').text(formatCurrency(invoice.amount), 450, totalsY, { align: 'right', width: 95 });

      doc.font('Helvetica-Bold').fillColor('#475569').text('Paid Amount:', 320, totalsY + 16);
      doc.font('Helvetica-Bold').fillColor('#10b981').text(formatCurrency(invoice.paid_amount), 450, totalsY + 16, { align: 'right', width: 95 });

      doc.font('Helvetica-Bold').fillColor('#475569').text('Outstanding Due:', 320, totalsY + 32);
      const dueColor = parseFloat(invoice.due_amount) > 0 ? '#ef4444' : '#10b981';
      doc.font('Helvetica-Bold').fillColor(dueColor).text(formatCurrency(invoice.due_amount), 450, totalsY + 32, { align: 'right', width: 95 });
      ...
      const bannerText = invoice.status === 'paid' 
      ...
      doc.moveTo(400, signatureY - 5).lineTo(545, signatureY - 5).strokeColor('#94a3b8').lineWidth(1).stroke();

      doc.moveDown(2);
      doc.fontSize(8).fillColor('#94a3b8').text('This is a verified digital payment receipt from Radha Govind Care Hospital.', { align: 'center' });

      doc.end();
    } catch (error) {
      console.error('PDF receipt export error:', error);
      return res.status(500).json({ error: 'Failed to generate PDF receipt', details: error.message });
    }
  }
  ...
  // ══════ VERIFY TOKEN FOR GENERAL CRUD ══════
    ...
    if (id) {
      ...
      if (req.method === 'PUT') {
        try {
          ...
          return res.status(500).json({ error: 'Failed to update invoice reconciliation', details: error.message });
        }
      }
    ...
    // ══════ COLLECTION OPERATIONS ══════
      ...
      if (req.method === 'POST') {
        try {
          const { patient_id, appointment_id, description, amount, status, payment_mode } = req.body;
